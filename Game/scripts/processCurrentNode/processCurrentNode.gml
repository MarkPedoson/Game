function processCurrentNode(_sizeAI) {
	...
	for(var i=max(0,curX-1);i<=min(oAStar.fieldWidth-1,curX+1);i++){
	    for(var j=max(0,curY-1);j<=min(oAStar.fieldHeight-1,curY+1);j++){
	        ...
	        if(diagonal){
				...
				if (canWalk){
					var deny = oAStar.clearence[getKey(i, j)] >= _sizeAI and
						oAStar.clearence[getKey(curX, j)] >= _sizeAI and
						oAStar.clearence[getKey(i, curY)] >= _sizeAI;
					canWalk = deny;
				}
	        }else{
	            canWalk=oAStar.walkable[i,j];
				...
				if (canWalk){
					var deny = oAStar.clearence[getKey(i, j)] >= _sizeAI;
					canWalk = deny;
				}
	        }
	        if(!closed && canWalk){
